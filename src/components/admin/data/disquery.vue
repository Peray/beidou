    ...
    <div class="disquery">
        ...
        <el-collapse-transition>
            <el-form status-icon :model="filterBeanForm" :inline="true" v-show="filterMenuShowing">
                ...
                <el-form-item>
                    <el-button type="primary" @click="queryDisMessageList">查询</el-button>
                </el-form-item>
            </el-form>
        </el-collapse-transition>
        ...
        <div class="bodyStyle">
            <el-table ref="bdCardTable" :data="respTableData"
                ...
                <el-table-column label="北斗号码" prop="sendCardId">
                </el-table-column>

                <el-table-column label="手机号码" prop="toMobile">
                </el-table-column>

                <el-table-column label="发送状态" prop="status">
                    <template slot-scope="scope">
                        {{scope.row.status == 0 ? '未发送':(scope.row.status == 1 ? '成功':'失败')}}
                    </template>
                </el-table-column>
            ...
            requestList: function (param) {
                this.$http.get(config.bdMessageQuery, {params: param}).then(response => {
                        if (response.ok) {
                            ...
                            if(typeof response.body.list !== 'un')
